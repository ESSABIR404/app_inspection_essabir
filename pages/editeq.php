...
$stmt = $conn->prepare("SELECT * FROM equipemnts WHERE ideq = :idequipe");
$stmt->execute([':idequipe' => $idequipe]);
$plan = $stmt->fetch(PDO::FETCH_ASSOC);
$tagequipe=$plan['tagequipe'];
$descriptioneq=$plan['descriptioneq'];
$typeeq=$plan['typeeq'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $nouveau_tagequipe = $_POST['nouveau_tagequipe'];
    $nouveau_descriptioneq = $_POST['nouveau_descriptioneq'];
    $nouveau_typeeq = $_POST['nouveau_typeeq'];

    // Mettre à jour les informations dans la base de données
    try {
        // Préparez et exécutez la requête de mise à jour
        $stmt = $conn->prepare("UPDATE equipemnts SET 
        tagequipe = :nouveau_tagequipe, 
        descriptioneq = :nouveau_descriptioneq, 
        typeeq = :nouveau_typeeq
        WHERE ideq = :idequipe");

$stmt->execute([
    ':nouveau_tagequipe' => $nouveau_tagequipe,
    ':nouveau_descriptioneq' => $nouveau_descriptioneq,
    ':nouveau_typeeq' => $nouveau_typeeq,
    ':idequipe' => $idequipe
]);
...
<nav class="navbar navbar-expand-lg bg-body-tertiary " >
    ...
    <section class="vh-100" style="background-color: #ffffff; background-size:contain;">
                                ...
                                <div class="card-body p-4 p-lg-5 text-black">
                                    <form action="" method="post">
                                        ...
                                        <div class="d-flex align-items-center mb-3 pb-1">
                                            <span class="h1 fw-bold mb-0" style="color: rgb(30, 1, 84);">Modifier l'équipement </span>
                                        </div>
        ...
        <div class="form-outline mb-4" >
        <label for="nouveau_tagequipe" class="form-label" style="color: rgb(54, 19, 120);"><strong> N° d'équipement :</strong></label>
        <input type="text" id="nouveau_tagequipe" name="nouveau_tagequipe" class="form-control form-control-lg" value="<?php echo $tagequipe; ?>"required>
        </div>

        <div class="form-outline mb-4" >
        <label for="nouveau_descriptioneq" class="form-label" style="color: rgb(54, 19, 120);"><strong>Description d'équipement :</strong></label>
        <input type="text" id="nouveau_descriptioneq" name="nouveau_descriptioneq" class="form-control form-control-lg" value="<?php echo $descriptioneq; ?>"required>
        </div>

        <div class="form-outline mb-4" >
        <label for="nouveau_typeeq" class="form-label" style="color: rgb(54, 19, 120);"><strong>Type d'équipement:</strong></label>
        <input type="text" id="nouveau_typeeq" name="nouveau_typeeq" class="form-control form-control-lg" value="<?php echo $typeeq; ?>"required>
        </div>
