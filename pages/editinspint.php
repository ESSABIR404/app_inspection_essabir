...
$stmt = $conn->prepare("SELECT * FROM inspections_interne WHERE id = :idinsp");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // Mettre à jour les informations dans la table `inspections_interne`
        $stmt = $conn->prepare("UPDATE inspections_interne SET  
                        tag = :nouveau_tag,
                        insp = :nouveau_insp,
        ...
        $stmt2 = $conn->prepare("UPDATE rapports SET
                        ...
                        methode_insp = :nouveau_methode_insp,
                        etat = :nouveau_etat
                        WHERE id_insp_ie = :idinsp && categorie = 'interne'");
...
<nav class="navbar navbar-expand-lg bg-body-tertiary " >
    ...
    <button type="button" onclick="window.location.href='inspection-interne.php'" class="btn btn-dark btn-lg" style="background-color:blueviolet; color: white";>Annuler</button>
               </div>    
                                    </form>
                                </div>
                            </div>
